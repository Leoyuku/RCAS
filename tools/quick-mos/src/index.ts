...
function refreshFiles() {
	...
	for (const [roId, oldT] of Object.entries<string>(runningOrderIds)) {
		if (oldT !== timestamp) {
			_.each(monitors, (monitor) => {
				monitor.onDeletedRunningOrder(roId)
			})
		}
	}
}
		...
		try {
			...
			if (filePath.match(/(\.ts|.json)$/)) {
				...
				if (fileContents.runningOrder) {
					...
					console.log('[DEBUG FETCH] S002 octext_storyTotalDur:', 
						ro.Stories.find((s: any) => s.ID === 'S002')?.octext_storyTotalDur
					)
