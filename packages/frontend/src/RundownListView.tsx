...
function ThumbnailPlaceholder({ type, isOnAir, isPreview }: {
    ...
    const borderColor = isOnAir ? 'rgb(206,0,24)' : isPreview ? 'rgb(0,195,91)' : 'rgba(255,255,255,0.1)'
        ...
        <div style={{
            ...
            {(isOnAir || isPreview) && (
                <div style={{
                    ...
                    inset: 0,
                    boxShadow: isOnAir
                        ? 'inset 0 0 12px rgba(206,0,24,0.3)'
                        : 'inset 0 0 12px rgba(0,195,91,0.3)',
                    pointerEvents: 'none',
                }} />
            )}
        </div>
...
function ColumnHeader() {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '44px 160px 1fr 56px 64px', padding: '0 8px', height: 26, alignItems: 'center', background: C.bgSurface, borderBottom: `1px solid ${C.borderStrong}`, flexShrink: 0 }}>
            {col('PG', 'center')}
            {col('SLUG', 'center')}
            {col('DUR', 'right')}
            {col('BACK', 'right')}
...
const StoryRowItem = forwardRef<HTMLDivElement, StoryRowItemProps>(
    ({ row, isOnAir, isPreview, isNext, isPlayed, runtime }, ref) => {
        ...
        return (
            <div
                ...
                style={{
                    display: 'grid',
                    gridTemplateColumns: '44px 160px 1fr 56px 64px',
                    padding: '0 8px',
                    alignItems: 'center',
                    justifyContent: 'center',
                    background: isOnAir
                    ...
                    {parts.length > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            {parts.map((part, partIdx) => {
                                ...
                                return (
                                    <div key={part._id as string} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
