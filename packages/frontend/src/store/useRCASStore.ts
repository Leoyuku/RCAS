...
interface RCASStore {
    ...
    tempParts: Record<string, { parts: Record<string, any>; order: string[] }>
    insertTempPart: (segmentId: string, sourceId: string, order: string[]) => void
    removeTempPart: (partId: string) => void
...
export const useRCASStore = create<RCASStore>((set) => ({
    ...
    _initSocket: () => {
        ...
        socket.on('runtime:tempParts', ({ tempParts }) => {
            set({ tempParts })
        })
    },
    ...
    insertTempPart: (segmentId: string, sourceId: string, order: string[]) => {
        if (!socket) return
        socket.emit('intent:insertTempPart', { segmentId, sourceId, order }, (result) => {
            if (!result?.ok) console.error('[Socket] INSERT TEMP PART failed:', result?.error)
        })
    },

    removeTempPart: (partId: string) => {
        if (!socket) return
        socket.emit('intent:removeTempPart', { partId }, (result) => {
            if (!result?.ok) console.error('[Socket] REMOVE TEMP PART failed:', result?.error)
        })
    },
