/**
 * @fileoverview This module, a direct port from the Sofie project, defines and manages the MosTime type.
 * It is responsible for parsing, validating, and stringifying timestamps in the specific format required by the
 * MOS protocol (e.g., YYYY-MM-DDTHH:mm:ss,sssZ).
 *
 * It depends on the `pad` function from the local `./lib.ts` module to correctly format date and time components.
 *
 * Key Functions:
 *
 * 1. Core Definition (`IMOSTime` interface): Establishes a nominal type for MOS timestamps, ensuring type safety.
 *
 * 2. Factory (`create` function): The primary entry point. It can create an IMOSTime object from various sources,
 *    including Date objects, numbers (timestamps), or strings in various formats.
 *
 * 3. Stringification (`stringify` function): The most critical output function. It takes an IMOSTime object and
 *    serializes it into the exact, MOS-compliant string format, using the imported `pad` function for zero-padding.
 *
 * 4. Type Guard (`is` function): Provides runtime type safety, allowing for reliable checks to determine if an
 *    object is a valid IMOSTime.
 *
 * 5. Parsers (`parseMosCustomFormat`, `parseTimeOffset`): Internal helpers to handle the complexity and variations
 *    of timestamp formats found in the MOS ecosystem, including timezone information.
 */
