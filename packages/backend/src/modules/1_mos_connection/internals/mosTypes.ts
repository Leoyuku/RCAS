/**
 * @fileoverview This file is a direct and crucial port from the Sofie project.
 * It serves as a sophisticated Façade and Factory for the entire MOS data type system.
 * Instead of just exporting types, it provides a central function, `getMosTypes`,
 * which constructs and delivers a configured set of type-handling utilities.
 *
 * This approach ensures consistent behavior (e.g., strict vs. non-strict error handling)
 * across all MOS data types used in the application.
 *
 * Key Components:
 *
 * 1. `getMosTypes(strict: boolean)`: The main factory function. It returns a `MosTypes` object
 *    containing configured handlers for each MOS data type.
 *
 * 2. `MosTypes` Interface: Defines the shape of the object returned by the factory, ensuring
 *    a consistent API for `mosString128`, `mosTime`, and `mosDuration` handlers.
 *
 * 3. `getMosType` (private helper): A generic function that wraps the raw MOS type modules
 *    (e.g., `mosString128.ts`) and injects the `strict` mode logic into their `create` and
 *    `validate` functions.
 *
 * 4. Direct Interface Exports (`IMOSString128`, etc.): The raw TypeScript interfaces are exported
 *    directly to allow for compile-time type checking in other modules.
 *
 * 5. Higher-Level Types (`IMOSExternalMetaData`): This file also defines more complex types
 *    that are built upon the foundational MOS types and the `AnyXMLValue` type from `xmlParse.ts`.
 */
