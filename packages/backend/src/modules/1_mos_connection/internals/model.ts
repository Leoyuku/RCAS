/**
 * @fileoverview This file is a direct port from the Sofie project and defines the core data models for the MOS protocol.
 * It acts as the central data contract, specifying the TypeScript interfaces for all major MOS objects and messages.
 *
 * The interfaces defined here are built upon the primitive MOS data types (e.g., IMOSString128, IMOSTime)
 * that are managed by the `mosTypes.ts` facade.
 *
 * Key Data Structures:
 * - IMOSRunningOrder: The top-level rundown object.
 * - IMOSStory: Represents a story within a rundown.
 * - IMOSItem: Represents a specific media item within a story.
 * - IMOSObject: Defines a media object (video, audio, etc.) in the MOS domain.
 * - IMOSAck: Defines the structure for acknowledgement messages.
 * - Status & Action interfaces: Define the shape of messages for status updates and actions.
 * - Enums: Provides enumerated types for standardized values like object types, statuses, and air readiness.
 */
