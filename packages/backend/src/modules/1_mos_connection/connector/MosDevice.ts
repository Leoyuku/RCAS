...
import { AnyXMLObject } from '../helper/mosModel'
import { MosModel } from '../helper'
import { MosReplyError } from '../helper/utils/Errors'
...
import { XMLMosListSearchableSchema,XMLMosObjectList,isXMLObject } from '../helper'
...
export class MosDevice implements IMOSDevice {
	...
	async sendMOSObject(obj: IMOSObject): Promise<IMOSAck> {
		...
		// 1058 行修改后：
		return this.handleParseReply((strict) => MosModel.XMLMosAck.fromXML('mosAck', reply.mos.mosAck, strict) as any)
	}
	...
	private async _sendAllMOSObjects(objs: IMOSObject[]): Promise<IMOSAck> {
		...
		const reply = await this.executeCommand(message)
		if (reply.mos) {
			const ack: IMOSAck = MosModel.XMLMosAck.fromXML('mosAck', reply.mos.mosAck, this.strict) as any
			return ack
		} else {
			throw this.unknownReply(reply)
		}
