...
import { getMosTypes } from '../../../internals/mosTypes'
...
<roID>49478285</roID>
	...
	test('ROReplaceItems: Smart Merge should protect metadata attributes', () => {
		// 1. 模拟内存中现有的、带有复杂元数据的项 (基于已有的 roItem00)
		const itemInMemory: IMOSItem = {
			...roItem00,
			MosExternalMetaData: [{
				MosScope: 'OBJECT' as any,
				MosSchema: 'vrt',
				MosPayload: { 
		...
		// 2. 模拟 NCS 发来的替换数据（比如只更新了 Slug，但 Payload 里的 techDescription 丢了）
		const incomingUpdate: IMOSItem = {
			...roItem00,
			Slug: mosTypes.mosString128.create('Updated Sport Slug'),
			MosExternalMetaData: [{
				MosScope: 'OBJECT' as any,
				MosSchema: 'vrt',
				MosPayload: { 
					frameRate: '60fps' // 此时只是字符串，丢失了对象结构
				}
			}]
		}
		...
		// 4. 验证：Smart Merge 成功“救回”了属性
		const payload = (result.MosExternalMetaData as any)[0].MosPayload
		expect(result.Slug).toEqual(mosTypes.mosString128.create('Updated Sport Slug'))
		expect(payload.frameRate.text).toBe('60fps')
		expect(payload.frameRate.techDescription).toBe('HFR') // 核心验证点
	})
})
