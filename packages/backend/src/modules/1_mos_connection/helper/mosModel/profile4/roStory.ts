...
import { IMOSROFullStory, IMOSItem } from '../../../internals' // Corrected path
	...
	get messageXMLBlocks(): XMLBuilder.XMLElement {
		...
		this.fullStory.Body.forEach((bodyItem) => {
			if (bodyItem.itemType === 'storyItem') {
				...
				if (item.Paths) XMLObjectPaths.toXML(xmlItem, item.Paths, this.strict)
				...
				if (item.MosObjects) {
					for (const mosObj of item.MosObjects) {
						XMLMosObjects.toXML(xmlItem, mosObj, this.strict)
					}
				}
			} else {
				// TODO: what is this?
