...
文档版本说明（v2.1 更新）
原始设计（v2.0）假设由团队自行实现 MOS 协议解析层（2_protocol_parsers、3_data_validators）。
经过实践验证，我们决定整体移植 Sofie Automation 的成熟 MOS 连接模块，
替代原计划自研的协议层。这是一个正确的工程决策：

协议解析、XML 处理、类型校验、ACK 状态机——Sofie 已用数年生产环境验证
我们的精力集中在 RCAS 的核心价值：数据转换（Blueprint）、播出引擎、设备控制

以下目录结构和流水线描述已更新为当前实际状态。


核心设计哲学：稳固基石，拥抱扩展（Solid Foundation, Embrace Extension）。
架构在物理和逻辑上都为未来变化预留了清晰的扩展点，核心永远保持稳定、封闭。
...
后端内部是一条单向流动的流水线，每一级有明确的职责边界：
packages/backend/src/
│
├── index.ts                          # 🚀 启动入口
│                                     #    加载 .env → 初始化 MOS → 启动 HTTP/WS → 注册优雅关闭
│
├── shared/                           # 🧱 共享基础设施（全局单例，无业务逻辑）
│   ├── logger.ts                     #    Winston 结构化日志（文件滚动 + 控制台）
│   ├── config.ts                     #    所有环境变量的唯一读取点
│   └── event-bus.ts                  #    [预留] 微服务模式下的模块间通信总线
│
├── modules/                          # 📦 业务流水线（数据从上到下单向流动）
│   │
│   ├── 1_mos_connection/             # ══ 第1站：MOS 接入层 ══（已完成 ✅）
│   │   │                             #    与 NCS 建立 MOS 协议连接，接收所有 Profile 0/2/4 消息
│   │   │                             #    基于 Sofie Automation 成熟 MOS 模块整体移植
│   │   ├── mos-connection.ts         #    [对外接口] MosConnector 类，注册所有回调，转发给 3_store
│   │   ├── connector/                #    [Sofie 移植] TCP 连接管理、MOS 协议状态机
│   │   ├── helper/                   #    [Sofie 移植] XML 解析、MOS 类型工具函数
│   │   └── internals/                #    [Sofie 移植] MOS 协议模型定义（类型、枚举）
│   │
│   ├── 2_ingest/                     # ══ 第2站：数据转换层 ══（待实现 🔲）
│   │   │                             #    将 MOS 格式（IMOSRunningOrder）翻译为 RCAS 内部格式（IRundown）
│   │   │                             #    这是 Blueprint 系统的核心所在
│   │   └── README.md                 #    职责说明（待实现时删除，替换为实际代码）
│   │
│   ├── 3_store/                      # ══ 第3站：状态管理层 ══（已完成 ✅）
│   │   │                             #    内存状态为主（毫秒级读写），JSON 文件持久化为辅
│   │   │                             #    通过 EventEmitter 向下游推送变更事件
│   │   ├── rundown-store.ts          #    核心状态管理器（Map + EventEmitter）
│   │   ├── json-persistence.ts       #    JSON 文件持久化（防抖写入 + 启动恢复）
│   │   └── socket-server.ts          #    Socket.io 实时推送（订阅 Store 事件 → 广播前端）
│   │
│   ├── 4_domain_engine/              # ══ 第4站：核心引擎（大脑）══（待实现 🔲）
│   │   │                             #    RCAS 与普通 MOS 客户端的核心区别所在
│   │   ├── blueprints/               #    数据转换规则（IRundown → Timeline，可热插拔）
│   │   ├── engine/                   #    播出状态机（Take / SetNext / Stop）
│   │   └── README.md
│   │
│   └── 5_playout_controllers/        # ══ 第5站：播出控制层（四肢）══（待实现 🔲）
│       │                             #    State Diff → 最小指令集 → 物理设备
│       ├── tsr-engine.ts             #    Timeline State Resolver（核心算法）
│       ├── drivers/                  #    设备驱动（CasparCG / ATEM / vMix / OSC...）
│       └── README.md
│
├── adapters/                         # 🔌 通信适配器层（待实现 🔲）
│   ├── in-memory-adapter.ts          #    [单体/EXE 模式] 内存 EventBus 桥接所有模块
│   └── grpc-adapter.ts               #    [微服务模式] gRPC 网络通信
│
└── entrypoints/                      # [预留] EXE 打包 / 微服务拆分时的独立入口
...
NCS (iNEWS / ENPS)
    ...
    ▼
┌─────────────────────────────────┐
│  1_mos_connection               │  接收 roCreate / roDelete /
│  （Sofie MOS 模块）              │  roElementAction 等全部协议消息
│  mos-connection.ts              │  → 所有回调均有异常捕获，保证 ACK 必达
└────────────┬────────────────────┘
             │ 调用 Store 方法
             ▼
┌─────────────────────────────────┐
│  3_store / rundown-store.ts     │  内存 Map 维护全量 RO 状态
│                                 │  严格遵循 MOS 3.6.12 协议语义
│                                 │  → 触发 EventEmitter 事件
└──────┬──────────────────┬───────┘
       │ 异步防抖写入       │ 实时事件
       ▼                  ▼
...
│ data/       │   │  连接时推送全量快照 │  → 断线重连自动恢复
│ rundowns/   │   └───────────────────┘
└─────────────┘
       │ 启动时恢复
       └──────────────────▶ rundown-store.restore()
完整目标流程（v3.0 规划）
NCS → 1_mos_connection → 3_store → 2_ingest（Blueprint）
                                        │
                                        ▼
                              4_domain_engine（Resolver）
                                        │ Timeline
                                        ▼
                              5_playout_controllers（TSR + Drivers）
                                        │
                                        ▼
                              物理设备（CasparCG / ATEM / vMix...）

第四部分：部署形态
两种形态使用同一套业务代码，差异仅在启动入口和通信适配器：
形态适用场景模块通信打包方式单体/EXE 模式单机广电工作站，双击运行内存直接调用pkg 打包成单个 .exe微服务模式大型台站，高可用部署gRPC 网络通信Docker 容器化，独立扩展

第五部分：关键技术决策（v2.1 更新）
决策项原始方案（v2.0）当前方案（v2.1）原因MOS 协议层自研整体移植 Sofie MOS 模块生产验证成熟，避免重复造轮子数据校验Zod（自研）Sofie 内置严格模式已覆盖全部 MOS 类型校验状态持久化MongoDB / PostgreSQLJSON 文件（当前）单机部署无需数据库依赖，人工可读可编辑日志自定义Winston（结构化日志）文件滚动、级别控制、生产/开发格式切换实时推送未定Socket.io前端连接即得全量快照，断线自动重连语言TypeScript StrictTypeScript Strict不变模块间通信EventBus（定义明确的接口）EventEmitter（当前）→ EventBus（微服务时）不变

第六部分：当前开发进度
模块                    状态                          说明
1_mos_connection      ✅ 完成        Profile 0/2/4 全部回调，异常捕获，多设备管理，优雅关闭
3_store               ✅ 完成        内存状态管理、JSON 持久化、Socket.io 实时推送
...
4_domain_engine       🔲 待实现       播出状态机、Timeline Resolver
5_playout_controllers 🔲 待实现       TSR 引擎、设备驱动adapters🔲 待实现微服务模式拆分时实现
